...
import { isNull, LocaleDefinition, UserLoginData, UnitOfWork, MenuData } from 'base-commons-module';
import { UserPrivilage } from 'core-client-commons';
...
export class SecurityRelatedUtil {
     ...
     * membaca data locale
     */
    static getLocales(): LocaleDefinition[] {
     ...
     */
    static getLoginData(callback: (data: UserLoginData) => any, loginDataErrorHandler?: (errorCode: string, errorMessage: string, exc: any) => any) {
        ...
        if (userInfoSwap == null) {
            ajaxhelper.generateOrGetAjaxUtils().get('/dynamics/rest-api/system/current-login-info.svc')
                .then((d: any) => {
                    let serverData: UserLoginData = d.user;
                    if (serverData == null) {
     ...
     */
    static getCurrentUnitOfWork(): UnitOfWork {
        return window[SecurityRelatedUtil.KEY_FOR_UNIT_OF_WORK];
    }
    /**
     * membaca unit of work dengan promise
     */
    static getCurrentUnitOfWorkWithPromise(): Promise<UnitOfWork> {
        return new Promise<UnitOfWork>( (fullfill: (n: UnitOfWork) => any, reject: (exc: any) => any) => {
    ...
    static procesLoginDataFromCachedVariable(callbackOnNotLogIn: () => any) {
        let loginInfo: UserLoginData = window[SecurityRelatedUtil.USER_LOGIN_INFO_HOLDER_KEY] || null;
        if (loginInfo == null) {
            callbackOnNotLogIn();
            return;
        }
     ...
     */
    static processLoginData(loginData: UserLoginData) {
     ...
     */
    static getCachedLoginData(): UserLoginData {
        let userInfoSwap: any = window[SecurityRelatedUtil.USER_LOGIN_INFO_HOLDER_KEY] || null;
        if (userInfoSwap == null) {
     ...
     */
    static indexMenuData(data: UserLoginData) {
        let indexerBaru: { [id: string]: UserPrivilage } = {};
        let indexerMenu: { [id: string]: MenuData } = {};
        let idxPrivByCode: { [id: string]: UserPrivilage } = {};
        let idxMenuByCode: { [id: string]: MenuData } = {};
    ...
    static getMenuById(id: string | number) {
        id = id + "";
        let swapId: any = id;
        let indexerMenu: { [id: string]: MenuData } = window["indexedMenu"] || null;
        if (indexerMenu == null) {
            return null;
        }
     ...
     */
    static getCurrentPrivilage(menuId: number): UserPrivilage {
        let indexerBaru: { [id: string]: UserPrivilage } = window["indexedPrivilage"] || {};
     ...
     */
    static getCurrentPrivilageByCode(menuCode: string): UserPrivilage {
        let indexerBaru: { [id: string]: UserPrivilage } = window["indexedPrivilageByCode"] || {};
     ...
     */
    static getMenuByCode(menuCode: string): MenuData {
        let indexerBaru: { [id: string]: MenuData } = window["indexedMenuByCode"] || {};
        return indexerBaru[menuCode];
    }
     ...
     */
    static appendMenu(menu: MenuData) {
        let indexerBaru: { [id: string]: UserPrivilage } = {};
        let indexerMenu: { [id: string]: MenuData } = {};
        let idxPrivByCode: { [id: string]: UserPrivilage } = {};
        let idxMenuByCode: { [id: string]: MenuData } = {};
