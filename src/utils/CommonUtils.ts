...
import { isNull , cloneObject, SortParam  } from 'base-commons-module'; 
    ...
    if ( tgName  === 'input') {
        ...
        if ( selElem.multiple) {
            ...
            for ( let idx = 0 ; idx < opts.length ; idx++) {
                let opt: HTMLOptionElement|null = opts.item(idx); 
                if  ( opt &&  opt.selected) {
                    rtvl.push(opt.value) ;
                }
            }
 ...
 */
export function isSortExistOnArray ( sortParam: SortParam , arrayOfSorts: SortParam[]  ): boolean  {
    if ( isNull(sortParam) ) {
        return false ; 
    }
