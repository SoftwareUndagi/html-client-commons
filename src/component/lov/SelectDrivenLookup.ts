import { LOVEnabledComponent } from "core-client-commons";
...
export class SelectDrivenLookup implements LOVEnabledComponent {
     ...
     */
    additionalTaskOnLookupDataRecieved: (lookupData: CommonLookupValue[]) => any ;
    /**
     * filter data, kalau di perlukan untuk memfilter data. mana yang tampil, mana yang tidak. kalau boleh tampil, return true ;
     */
    private _dataFilter: (data: CommonLookupValue) => boolean ;
    ...
    constructor(element: HTMLElement, lovId: string, pattern?: string) {
        ...
        this._dataFilter = (data: CommonLookupValue): boolean => {
            return true ;
        };
    }  
    /**
     ...
     */
    assignLookupData(lookupData: CommonLookupValue[]) {
     ...
     */
    set DataFilter ( filter: (data: CommonLookupValue ) => boolean ) {
