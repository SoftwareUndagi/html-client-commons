...
export class EditorComboboxLabeled extends BaseHtmlComponent<EditorComboboxLabeledProps , EditorComboboxLabeledState>  implements EditorInputElement {
    ...
    render () {
        ...
        if ( !readonly) {
            ...
            let x: (lookup: CommonCommunicationData.CommonLookupValue) => string = isNull( this.props.readonlyLabelFormatter) ? EditorCombobox.FORMATTER_READONLY : this.props.readonlyLabelFormatter!; 
            if (!isNull(this.state.value) && this.state.value.length > 0) {
                let lks: CommonCommunicationData.CommonLookupValue[] = this.props.lookupContainers[this.props.lookupCode] ; 
                if ( !isNull(lks)) {
                    for ( let l of lks ) {
