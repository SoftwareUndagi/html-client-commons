...
import { i18n , readNested, setValueHelper } from '../../../utils/index';
...
export interface EditorSelect2LabeledProps  extends BaseSearchableComboBoxProps {
    /**
...
export interface EditorSelect2LabeledState extends BaseSearchableComboBoxState {
    /**
 ...
 */
export class EditorSelect2Labeled extends BaseSearchableComboBox<EditorSelect2LabeledProps, EditorSelect2LabeledState>/*BaseSearchableComboBox<EditorSelect2LabeledProps, EditorSelect2LabeledState>*/ implements EditorInputElement, CustomMandatoryValidateEnabled {
    
    /**
     * method untuk force read data dari control
     */
    forceReadDataFromControlMethod:   (doNotUpdateState ?: boolean) => any ;
    constructor(props: EditorSelect2LabeledProps ) {
        super(props) ; 
    ...
    runMandatoryValidation(): CustomValidationFailureResult {
        ...
        if ( !isNull(rtvl)) {
            this.setStateHelper(c => c.mandatoryValidationFailed = true  );
        }
        return rtvl ; 
    }
    assignDataToControl(data: any, updateState?: boolean) {
        let val: any = isNull(this.props.customFetchValueFromData) ?  (readNested(data , this.props.fieldName )  || null) : this.props.customFetchValueFromData!(data);  // readNested(data , this.props.fieldName);
        if ( !isNull(val )) {
            if ( !isNaN(val)) {
                val = val + '' ; 
            }
    ...
    generateDefaultState(): EditorSelect2LabeledState {
        return {
            value : null! , 
            valueLookup : null !, 
            valueRendered : null !, 
    ...
    render (): JSX.Element {
        if ( !isNull(this.props.readonlyState) && this.props.readonlyState ) {
            ...
            <LabelOnlyEditorControl 
                ...
                cssLabel={this.props.cssLabel}
                cssDivTextbox={this.props.cssDivTextbox}
                value={ this.state.mandatoryValidationFailed ?  
                <span {...propSPan}>{this.renderUnPopupControl()}</span> : 
                this.renderUnPopupControl()}
            /> );
        }
    }
}